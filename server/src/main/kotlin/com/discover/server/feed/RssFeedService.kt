...
class RssFeedService(private val restTemplate: RestTemplate, private val feedInput : SyndFeedInput) {
    ...
    fun getFeedBySite(resource: String): Set<RssFeedItem> {
        ...
        return if (feed.entries.isNotEmpty()) {
            feed.entries.map {
                RssFeedItem(title = it.title, description = it.description.value,
                        url = it.link, timePublished = it.publishedDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime(), origin = resource)
            }.toSet()
        } else {
            emptySet()
        }
